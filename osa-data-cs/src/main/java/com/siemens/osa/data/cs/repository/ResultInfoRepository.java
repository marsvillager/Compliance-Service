...
public interface ResultInfoRepository extends JpaRepository<ResultInfo, Long> {
    ...
    @Query(value = "select * from result where host_ip = inet(:hostIp) and timestamp = :collectTime", nativeQuery = true)
    List<ResultInfo> getResultByHostIpTime(@Param("hostIp") String hostIp, @Param("collectTime") Timestamp collectTime);

    @Query(value = "select * from result where timestamp = (select max(timestamp) from result where timestamp >= :beginTime and timestamp < :endTime)", nativeQuery = true)
    List<ResultInfo> getRecentResultWithZone(@Param("beginTime") Timestamp beginTime, @Param("endTime") Timestamp endTime);

//    @Query(value = "insert into result(timestamp, id, os, lang, server_ip, host_ip, rule_id, expected, actual, status) " +
//            "values(timestamp = :timestamp, id = :id, os = :os, lang = :lang, server_ip = :serverIp, hostIp = :host_ip, " +
//            "rule_id = :ruleId, expected = :expected, actual = :actual, status = :status", nativeQuery = true)
...
//    @Query(value = "insert into result(timestamp, id, os, lang, server_ip, host_ip, rule_id, status) " +
//            "values(:timestamp, :id, :os, :lang, :serverIp, :host_ip, " +
//            ":ruleId, :status", nativeQuery = true)
//    void addResult(@Param("timestamp") Timestamp timestamp, @Param("id") Integer id, @Param("os") String os,
//                   @Param("lang") String lang, @Param("serverIp") String serverIp, @Param("hostIp") String hostIp,
//                   @Param("ruleId") String ruleId, @Param("status") String status);

    @Query(value = "insert into result(timestamp, id, os, lang) values(:#{#timestamp}, :#{#id}, :#{#os}, :#{#lang})", nativeQuery = true)
    void addResult(@Param("timestamp") Timestamp timestamp, @Param("id") Integer id, @Param("os") String os, @Param("lang") String lang);
}
